...
class ConfigModal(Modal, title="Configure Shop Item"):
  price = TextInput(label="Price", placeholder="ex: 100", required=True)
  quantity = TextInput(label="Quantity", placeholder="ex: 1", required=True, default="1")
  quality = TextInput(label="Quality (optional)", required=False, placeholder="ex: 1-25 for item quality", default="1")
  is_blueprint = TextInput(label="Blueprint? 1 = Yes, 0 = No", required=False, placeholder="0/No is default", default="0")
  ...
  async def on_submit(self, interaction: discord.Interaction):
    ...
    async with v.pool.acquire() as con:
      ...
      qual = int(str(self.quality)) if str(self.quality) else 1
    ...
    await interaction.response.edit_message(
      content=f"Item Added: **{name}** to the shop (price: {price}, qty: {qty}).",
      view=None
...
class ShopAddView(View):
  ...
  async def start(self, interaction: discord.Interaction):
    async with self.pool.acquire() as con:
      cats =  await search_categoies(con, q="", limit=25)
    self.clear_items()
